...
public class QueueServerActionType implements InteractionActionType<QueueServerAction>, InteractionCommandHandler {
    private final BungeeConnector bungeeConnector;

    public QueueServerActionType(BungeeConnector bungeeConnector) {
        this.bungeeConnector = bungeeConnector;
    }

    @Override
    public String serialize(QueueServerAction obj) {
    ...
    @Override
    public QueueServerAction deserialize(String str) {
        String[] split = str.split(";");
        InteractionType type = split.length > 2 ? InteractionType.valueOf(split[2]) : InteractionType.ANY_CLICK;
        return new QueueServerAction(bungeeConnector, new String(Base64.getDecoder().decode(split[0]), StandardCharsets.UTF_8), type, Long.parseLong(split[1]), Long.parseLong(split.length > 3 ? split[3] : "0"));
    }

    @Override
    public Class<QueueServerAction> getActionClass() {
        return QueueServerAction.class;
    }
    ...
    public InteractionActionImpl parse(CommandContext context) throws CommandExecutionException {
        ...
        return new QueueServerAction(bungeeConnector, server, type, cooldown, delay);
    }
