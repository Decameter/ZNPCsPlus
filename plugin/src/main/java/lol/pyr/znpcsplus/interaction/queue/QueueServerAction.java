...
public class QueueServerAction extends InteractionActionImpl {
    private final String server;
    private final BungeeConnector bungeeConnector;

    public QueueServerAction(BungeeConnector bungeeConnector, String server, InteractionType interactionType, long cooldown, long delay) {
    ...
    public void run(Player player) {
        bungeeConnector.queuePlayer(player, server);
    }
        ...
        return Component.text(index + ") ", NamedTextColor.GOLD)
                .append(Component.text("[EDIT]", NamedTextColor.DARK_GREEN)
                        ...
                        .clickEvent(ClickEvent.clickEvent(ClickEvent.Action.SUGGEST_COMMAND,
                                "/" + context.getLabel() + " action edit " + id + " " + index + " queueserver " + getInteractionType().name() + " " + getCooldown()/1000 + " " + getDelay() + " " + server))
                        ...
                        .append(Component.text(" | ", NamedTextColor.GRAY))
                        .append(Component.text("Queue Server: ", NamedTextColor.GREEN)
                                .hoverEvent(HoverEvent.hoverEvent(HoverEvent.Action.SHOW_TEXT,
